...
const createPayment = async (req, res) => {
  try {
    ...
    if (!payment) {throw new Error("Something went wrong, please try again or later!")}
    res.status(200).json({
      success: true,
...
const getPaymentDetails = async (req, res) => {
  try {
    ...
    if (!getDetails) {throw new Error("Payment not found!")}
    res.status(200).json({
      success: true,
...
const updateDetails = async (req, res) => {
  try {
    ...
    if (!paymentExists) {throw new Error("Payment not found!")}
    await paymentService.updateDetails(paymentId, reqBody);
